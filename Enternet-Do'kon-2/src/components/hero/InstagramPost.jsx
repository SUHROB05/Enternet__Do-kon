        ...
        {instagramPosts.map((post) => (
          <>
            <div className="h-72 lg:h-80 overflow-hidden " key={post.id}>
              <img
                src={post.image} alt=""
                className=" w-full h-full object-cover transition-transform ease-in-out hover:scale-125 hover:-rotate-12 "
              />
            </div>
          </>
        ))}
          ...
          <>
            <div className="flex items-center gap-8 p-8 py-12 relative">
              ...
              <div>
                ...
                <Title level={1} className="absolute -bottom-5 ring-0 opacity-10">0{post.id}</Title>
              </div>
            </div>
          </>
        ))}
